...
/**
 * same class as {@link OverlayManager} except that scope used is test
 */
public class TestOverlayManager {
     ...
     */
    public TestOverlayManager(List overlays, MavenProject project, String defaultIncludes, String defaultExcludes,
            Overlay currentProjectOverlay) throws InvalidOverlayConfigurationException {
    ...
    private boolean compareOverlayWithArtifact(Overlay overlay, Artifact artifact) {
        return (StringUtils.equals(overlay.getGroupId(), artifact.getGroupId())
                ...
                && StringUtils.equals(overlay.getType(), artifact.getType()) && StringUtils.equals(
                overlay.getClassifier(), artifact.getClassifier()));
    }
